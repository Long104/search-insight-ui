...
test.describe("Kalifind Search Functionality Tests", () => {
  ...
  test("2. Autocomplete Suggestions - Typing shows dropdown with loading state", async ({
    page,
  }) => {
    const searchInput = page.locator('input[placeholder="Search"]');
  ...
  test("14. Loading States - Skeleton loaders during search", async ({ page }) => {
    ...
    const _hasSkeleton = (await skeletonLoaders.count()) > 0;
    const _hasLoadingText = await loadingText.isVisible();
